...
export default {
  async fetch(request, env, ctx) {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    if (request.method === 'POST' && url.pathname === '/api/check-user') {
      try {
        const { username, email, tournamentType } = await request.json();

        if (!username || !email || !tournamentType) {
          return new Response(
            JSON.stringify({ error: 'Missing required fields: username, email, tournamentType' }),
            { status: 400, headers: { ...headers, 'Content-Type': 'application/json' } }
          );
        }
        ...
        // Check if there is already a purchase for this user in this tournament
        ...
        return new Response(
          JSON.stringify({ exists: !!existingUserPurchase }),
          { status: 200, headers: { ...headers, 'Content-Type': 'application/json' } }
        );
    ...
    if (request.method === 'POST' && url.pathname === '/api/verify') {
      try {
        ...
        // 2. Check if a passcode has already been issued to the same username, email, and tournament_id
        const existingUserPurchase = await env.DB.prepare(
          'SELECT passcode FROM purchases WHERE username = ? AND email = ? AND tournament_id = ?'
        ).bind(username, email, tournamentId).first();
        ...
        if (existingUserPurchase) {
          return new Response(
            JSON.stringify({ error: 'You already have a passcode for this tournament. Please check your email or contact the admin for assistance.' }),
            { status: 400, headers: { ...headers, 'Content-Type': 'application/json' } }
          );
        }

        // 3. Verify payment with Flutterwave (or bypass for free tournaments)
        const QUICK_CHALLENGE_PRICE = Number(env.QUICK_CHALLENGE_PRICE ?? 200);
        const WEEKEND_CHALLENGE_PRICE = Number(env.WEEKEND_CHALLENGE_PRICE ?? 500);
        ...
        if (expectedAmount === 0 && transactionId.startsWith('FREE_')) {
          // Free tournament — skip Flutterwave entirely
          flwData = {
            status: 'success',
            data: { status: 'successful', amount: 0, currency: 'NGN' }
          };
        } else {
        ...
        // 4. Get passcode directly from NAW_DB (naw-passcodes-db) D1
        if (!env.NAW_DB) {
          return new Response(
        ...
        // Query unused passcodes for this tournament from the "code" table
        const { results: unusedCodes } = await env.NAW_DB.prepare(
          'SELECT passcode FROM code WHERE tournament_id = ? AND (is_used = 0 OR is_used IS NULL) ORDER BY id ASC'
        ).bind(tournamentId).all();
        ...
        // Find the first passcode that is NOT already purchased in DB purchases table
        let passcode = null;
        for (const row of unusedCodes) {
          const exists = await env.DB.prepare(
            'SELECT 1 FROM purchases WHERE passcode = ?'
          ).bind(row.passcode).first();
        ...
        // 5. Save to D1 (purchases)
        try {
          await env.DB.prepare(
            'INSERT INTO purchases (transaction_id, username, email, tournament_id, passcode, amount, currency) VALUES (?, ?, ?, ?, ?, ?, ?)'
          ).bind(transactionId, username, email, tournamentId, passcode, flwData.data.amount ?? 0, flwData.data.currency).run();
        } catch (dbErr) {
          return new Response(
            JSON.stringify({ error: `Failed to save purchase: ${dbErr.message}` }),
            { status: 500, headers: { ...headers, 'Content-Type': 'application/json' } }
          );
        }

        // 6. Mark passcode as used in env.NAW_DB (code table)
        try {
          const nowStr = new Date().toISOString().replace('T', ' ').substring(0, 19);
          await env.NAW_DB.prepare(
            'UPDATE code SET is_used = 1, used_by_username = ?, used_by_email = ?, used_at = ? WHERE passcode = ? AND tournament_id = ?'
          ).bind(username, email, nowStr, passcode, tournamentId).run();
        } catch (dbErr) {
          console.error('Failed to update NAW_DB code status:', dbErr);
        }
        ...
        const emailHtml = `
          <div style="font-family: sans-serif; padding: 24px; color: #333; max-width: 600px; line-height: 1.6;">
            ...
            <table style="width: 100%; border-collapse: collapse; margin-bottom: 24px; font-size: 14px;">
              ...
              <tr>
                <td style="padding: 8px 0; border-bottom: 1px solid #f3f4f6; color: #6b7280;">Ref:</td>
              ...
              <tr>
                ...
                <td style="padding: 8px 0; border-bottom: 1px solid #f3f4f6; font-weight: bold; text-align: right; color: #111827;">&#x20A6;${flwData.data.amount ?? 0}</td>
              </tr>
            </table>
            <p style="background-color: #fffbeb; border: 1px solid #fef3c7; border-radius: 8px; padding: 12px; font-size: 13px; color: #b45309; margin-bottom: 24px;">
              💡 <strong>How to join:</strong> Copy the code above, open the <a href="https://play.google.com/store/apps/details?id=com.naijaayo.worldwide" style="color: #dc2626; font-weight: bold; text-decoration: underline;" target="_blank">Naija Ayo Worldwide</a> mobile app, go to the tournament arena, select your challenge type, and paste the code to enter the lobby.
            </p>
